...
#1 read in files
meta_data <- read.csv("dataset/metadata.csv", 
                      header = T,sep = ",")
file_paths <- dir_ls(paste("dataset/output_tsv/",input2,sep = ""))
file_contents <- list()
...
for (i in seq_along(posi)){
  ...
  if (length(levels(as.factor(man.data$Tumor)))==2){
...
#sig.data <- data.frame(NULL)#create a dataframe with 0 columns and 0 rows
...
#############data normality test########
# posi <- unique(outdat_all$position)
# for (i in seq_along(posi)){
#   sub.data.normal <- subset(outdat_all.sub,position==as.character(posi[i])&Tumor=="Normal")
#   sub.data.cancer <- subset(outdat_all.sub,position==as.character(posi[i])&Tumor=="Cancer")
#   sub.data.normal[,2] <- remove_outliers(sub.data.normal$mean)
#   sub.data.cancer[,2] <- remove_outliers(sub.data.cancer$mean)
#   man.data <- na.omit(rbind(sub.data.normal,sub.data.cancer))
#   norcheck <- normalTest((man.data$mean))
#   pval <- norcheck@test$p.value
#   if (pval<0.05){
#     print(paste("The normality of position",i,"is bad",pval))
#   }
# }
